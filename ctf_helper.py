...
def get_ctf_suggestions(technique_results: list[dict], mode: str = "ctf") -> list[str]:
    ...
    if lsb and lsb["score"] >= STRONG_THRESHOLD:
        ...
        if is_ctf:
            ...
            suggestions.append(
                "🛡️ **LSB Irregularity (High Confidence stego detected).** Action Plan:\n"
        ...
        if is_ctf:
            ...
            suggestions.append(
                "🛡️ **Moderate LSB Variance.** Slight pixel noise imbalance. Check for low-capacity data stuffing or minor watermarking metadata."
            )
    ...
    if meta and meta["score"] >= MODERATE_THRESHOLD:
        ...
        if is_ctf:
            ...
            suggestions.append(
                "🛡️ **Exif Metadata Tampering.** Suspicious headers detected:\n"
    ...
    if ela and ela["score"] >= MODERATE_THRESHOLD:
        ...
        if is_ctf:
            ...
            suggestions.append(
                "🛡️ **Error Level Analysis Alert.** Splicing detected:\n"
    ...
    if spectro and spectro["score"] >= MODERATE_THRESHOLD:
        ...
        if is_ctf:
            ...
            suggestions.append(
                "🛡️ **High-Frequency Spectrogram Anomaly.**\n"
    ...
    if audio_lsb and audio_lsb["score"] >= MODERATE_THRESHOLD:
        ...
        if is_ctf:
            ...
            suggestions.append(
                "🛡️ **Covert Audio Channel.** Audio LSB bitstream manipulation:\n"
    ...
    if eof and eof["score"] >= MODERATE_THRESHOLD:
        ...
        if is_ctf:
            ...
            suggestions.append(
                f"🛡️ **Appended Trailer Bytes Alert ({trailing_size:,} bytes).**\n"
    ...
    if binw and binw["score"] >= MODERATE_THRESHOLD:
        ...
        if is_ctf:
            ...
            suggestions.append(
                "🛡️ **Polyglot / Embedded Container detected.**\n"
    ...
    if strs and strs["score"] >= MODERATE_THRESHOLD:
        ...
        if is_ctf:
            ...
            suggestions.append(
                "🛡️ **High-Suspicion Strings Found.**\n"
    ...
    if stegh and stegh["score"] >= MODERATE_THRESHOLD:
        ...
        if is_ctf:
            ...
            suggestions.append(
                "🛡️ **Password LSB Steganography Warning.**\n"
    ...
    if not suggestions:
        if is_ctf:
            ...
            suggestions.append(
                "🛡️ **File structure matches clean baselines.**\n"
